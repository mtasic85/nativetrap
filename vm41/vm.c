...
object_t * frame_exec(struct frame_t * frame) {
    ...
    #define MAKE_OP_JMP(NEXT_OP) \
        inst = &insts->items[pc]; \
        pc += inst->operands.i.a; \
        NEXT_OP;

    #define MAKE_OP_JEQ(NEXT_OP) \
        inst = &insts->items[pc]; \
    ...
    while (1) {
        switch (inst->opcode.name) {
            ...
            case OP_MOV:
                ...
                break;
            case OP_ADD:
                MAKE_OP_ADD(MAKE_OP_EMPTY();
                break;
            case OP_LT:
                MAKE_OP_LT(MAKE_OP_EMPTY();
                    // MAKE_OP_JEQ(;)
                //         MAKE_OP_ADD(
                //             MAKE_OP_JMP(;)
                //         )
                ...
                break;
            case OP_JMP:
                MAKE_OP_JMP(MAKE_OP_EMPTY();
                break;
            case OP_JEQ:
                MAKE_OP_JEQ(MAKE_OP_EMPTY();
                break;
            default:
                ;
        }
